...
class GSKTool:
    def __init__(self, master):
        ...
        self.begin_button = Button(self.bottom_frame, text='Begin!', command=self.being_taks)
        ...
        self.browse1.grid(row=1, column=2, padx=10, sticky=E)

        self.output_path_label.grid(row=2, column=0, pady=5, sticky=E)
        self.output_entry.grid(row=2, column=1, pady=5, sticky=E)
        self.browse2.grid(row=2, column=2, padx=10, sticky=E)

        self.begin_button.grid(row=3, column=1, ipadx=10, pady=10, padx=(10, 30), sticky=NSEW)
    ...
    def data_automation(self):
        ...
        self.complete_dict.update(collected_data)

        keys = list(self.complete_dict.keys())
        print(keys)

        new_output = pd.DataFrame.from_dict(self.complete_dict, orient='index')
